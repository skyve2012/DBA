...
class CMNISTV2(SubpopDataset):
    N_STEPS = 20001 # old 5001
    CHECKPOINT_FREQ = 250
    INPUT_SHAPE = (3, 224, 224,)
    data_type = "images"

    def __init__(self, data_path, split, hparams, train_attr='yes', subsample_type=None, duplicates=None):
        root = Path(data_path)/'cmnistv2'
        if split=='tr':
            self.align = sorted(glob(os.path.join(str(root), hparams['cmnistv2_difficult'], 'align',"*","*")))
            self.conflict = sorted(glob(os.path.join(str(root), hparams['cmnistv2_difficult'], 'conflict',"*","*")))
            self.data = self.align + self.conflict
        elif split=='va':
            self.data = sorted(glob(os.path.join(str(root), hparams['cmnistv2_difficult'],'valid',"*")))
        elif split=='te':
            self.data = sorted(glob(os.path.join(str(root), 'test',"*","*")))
        else:
            raise NotImplementedError
        ...
        self.a = []
        self.x = []
        for cur_i in tqdm.tqdm(self.idx):
            img = torch.tensor(np.array(Image.open(self.data[cur_i]).convert('RGB')).astype(np.float32)) / 255.
            img = img.permute(2, 0, 1)
            self.x.append(img)
            self.y.append(int(self.data[cur_i].split('_')[-2]))
            self.a.append(int(self.data[cur_i].split('_')[-1].split('.')[0]))
        if train_attr == 'yes':
    ...
    def __getitem__(self, index):
        if self.hparams['attr2weights']: # meaning attributes become weights now 
            i = self.idx[index]
            x = self.transform(self.x[i])
            y = torch.tensor(self.y[i], dtype=torch.long)
            a = torch.tensor(self.a[i], dtype=torch.float)
            return i, x, y, a
        else:
